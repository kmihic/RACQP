...
function model = remove_embedded_slacks(model)
  ...
  for ii = sp
    ...
    if(length(ix) == 1 && model.Aeq(ix,ii) == 1)
      ...
      if(isinf(model.lb(ii)) && isinf(model.ub(ii))) %have unbounded "slack", remove the constraint
        continue;
      elseif(isinf(model.lb(ii)) && isinf(model.ub(ii))) %"negative" slack, i.e. Ax>=b
        model.Aeq(:,ii) = -model.Aeq(:,ii);
        model.beq(:,ii) = -model.beq(ii);
      end
