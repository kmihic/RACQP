function sol = rp_factorize_admm_lasso(X, y, block_num, alpha, lambda_penalty, lambda_admm, max_iter)

%  RP ADMM with prefactorize
%  Need lambda_admm to be constant
%
...
store_f_A_Sub_A_sub=cell(1,block_num);
for i_block = 1:1:block_num
    if i_block == block_num
        ...
        store_f_A_Sub_A_sub{i_block} = chol(store_A_Sub_A_sub{i_block} + lambda_admm*speye(block_size_n));
        
    else
        A_sub = A(:,order(((i_block - 1)*block_size_r + 1):i_block*block_size_r));
        store_A_Sub_A_sub{i_block} = (A_sub'*A_sub);
        store_f_A_Sub_A_sub{i_block}  = chol(store_A_Sub_A_sub{i_block} + lambda_admm*speye(block_size_r));
    end
end
...
%  H_current = H_current + lambda_admm(num_iter+1)*speye(block_size);
  c_current = c_current_sub + c_res(x_update_index) - lambda_admm*x_k_2(x_update_index); 
  ...
  R_current = store_f_A_Sub_A_sub{current_block_index};
  sub_model_time = toc(sub_model_time_start) + sub_model_time;
  
  sub_solver_time_start = tic;
  results_x =  R_current\(R_current'\right_side); 
  ...
  %update x_k_2
  penalty_co = 1/((1-alpha)*lambda_penalty(num_iter+1)+lambda_admm);
  x_k_2(x_update_index) = penalty_co*soft( y_k(x_update_index) - lambda_admm*x_k(x_update_index) ,lambda_penalty(num_iter+1)*alpha);   
