...
function out = make_blocks(n_vars,groups, block_sizes, no_split)
  ...
  n_sum = 0;
  while (true) 
    while(b_ix <= n_blocks && length(groups{grp,1})>0)
    ...
    if(s>0) 
      %if overlapped block
      if(length(o_blocks) > 0)
        error('Not done. Verify if find works correctly; replace with any');
        [oo o_ix] = find(b_map == o_blocks'); 
        lo_ix = length(o_ix);
        if(lo_ix > 0 ) 
          grp = o_ix(randi(lo_ix));
        else
          grp = randi(s);
        end
      else
