...
function rac_out = rac_mip(model, run_mip, time_start)
  ...
  % no dual vars
  run_sub.use_dual_res = false;
  run_sub.do_rel_tol = false;
  ...
  while(~terminate(run_mip, curr_iter, cnt_perturb, time_start))
    ...
    res_curr = rac_current.sol_res_p;
  ...
  rac_out.sol_res_p.abs = -inf;
  rac_out.sol_res_d.abs = -inf;
  rac_out.sol_res_p.rel = res_best;
  rac_out.sol_res_d.rel = -inf;
  rac_out.sol_res_p.L2 = -inf;
  rac_out.sol_res_d.L2 = -inf;
